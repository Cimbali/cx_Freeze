...
if platform:
    ...
    platform_support = set(["darwin", "linux", "mingw", "win32"])
    platform_yes = set([plat for plat in platform if not plat.startswith("!")])
    if platform_yes:
        platform_support = platform_yes
    platform_not = set([plat[1:] for plat in platform if plat.startswith("!")])
    if platform_not:
